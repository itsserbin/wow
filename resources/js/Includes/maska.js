...
class w {
    ...
    process(t, s, e = !0) {
        ...
        const {mask: i, escaped: c} = this.escapeMask(s), r = [], k = this.opts.tokens != null ? this.opts.tokens : {},
            l = this.isReversed() ? -1 : 1, d = this.isReversed() ? "unshift" : "push",
            g = this.isReversed() ? 0 : i.length - 1,
            C = this.isReversed() ? () => o > -1 && p > -1 : () => o < i.length && p < t.length,
            S = (f) => !this.isReversed() && f <= g || this.isReversed() && f >= g;
        let E, u = -1, o = this.isReversed() ? i.length - 1 : 0, p = this.isReversed() ? t.length - 1 : 0;
        for (; C();) {
            const f = i.charAt(o), h = k[f],
                M = (h == null ? void 0 : h.transform) != null ? h.transform(t.charAt(p)) : t.charAt(p);
...
const V = (n) => JSON.parse(n.replaceAll("'", '"')), T = (n, t = {}) => {
    ...
    }, y = (n) => n !== "" ? Boolean(JSON.parse(n)) : !0, L = (n) => n.startsWith("[") && n.endsWith("]") ? V(n) : n,
    x = (n) => {
...
class J {
    ...
    setMaskedValue(t, s) {
        ...
        const a = e.masked(s), i = e.unmasked(e.isEager() ? a : s), c = e.completed(s),
            r = {masked: a, unmasked: i, completed: c};
