...
import _sfc_main$1 from "./Modal-e6f19919.mjs";
import _sfc_main$3 from "./Table-2d4d3ee8.mjs";
import _sfc_main$2 from "./ReviewsLayout-c3fe9722.mjs";
import "./Form-f08dc11c.mjs";
import "./ContentLayout-150d801d.mjs";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    function fetch(page) {
      ...
      axios.get(route("api.reviews.product.index", { "page": state.value.currentPage })).then((response) => {
      ...
      if (can("destroy-reviews")) {
        swal({
          ...
          if (result.isConfirmed) {
            axios.delete(route("api.reviews.product.destroy", id)).then(() => {
              fetch();
    ...
    function onEdit(id, i) {
      if (can("edit-reviews")) {
        axios.get(route("api.reviews.product.edit", id)).then(({ data }) => {
    ...
    function onUpdate() {
      if (can("edit-reviews")) {
        axios.put(route("api.reviews.product.update", state.value.item.id), state.value.item).then(() => {
          modalFunction();
          fetch();
          swal({
            title: "Success!",
    ...
    function onCreate() {
      if (can("create-reviews")) {
        axios.post(route("api.reviews.product.create"), state.value.item).then(({ data }) => {
          modalFunction();
          state.value.item = {};
          fetch();
          swal({
    ...
    function publishReview(id) {
      axios.post(route("api.reviews.product.accept", id)).then(() => {
        fetch();
        swal({
          title: "Success!",
          icon: "success"
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Content/Reviews/Products/Index.vue");
