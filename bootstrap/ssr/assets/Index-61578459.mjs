...
import _sfc_main$1 from "./Modal-76a6b308.mjs";
import _sfc_main$2 from "./OptionsLayout-7e4c3b00.mjs";
import _sfc_main$4 from "./Table-3de8e83a.mjs";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    function fetch(page) {
      ...
      axios.get(route("api.sizes.index", { "page": state.value.currentPage })).then((response) => {
      ...
      if (can("destroy-sizes")) {
        swal({
          ...
          if (result.isConfirmed) {
            axios.delete(route("api.sizes.destroy", id)).then(() => {
              fetch();
    ...
    function onEdit(id, i) {
      if (can("edit-sizes")) {
        axios.get(route("api.sizes.edit", id)).then(({ data }) => {
    ...
    function onUpdate() {
      if (can("edit-sizes")) {
        axios.put(route("api.sizes.update", state.value.item.id), state.value.item).then(() => {
          modalFunction();
          fetch();
          swal({
            title: "Success!",
    ...
    function onCreate() {
      if (can("create-sizes")) {
        axios.post(route("api.sizes.create"), state.value.item).then(() => {
          modalFunction();
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$2, mergeProps({ title: "Переклади" }, _attrs), {
        header: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(` Переклади `);
          } else {
            return [
              createTextVNode(" Переклади ")
            ];
          }
        }),
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (!state.value.isLoading && unref(can)("show-translations")) {
              _push2(`<div${_scopeId}>`);
              if (unref(can)("create-translations")) {
              ...
              _push2(ssrRenderComponent(_sfc_main$4, {
                data: state.value.sizes.data,
                onOnEdit: onEdit,
                onOnDestroy: onDestroy,
                canDestroy: unref(can)("destroy-sizes")
              }, null, _parent2, _scopeId));
              _push2(ssrRenderComponent(_sfc_main$5, {
                pagination: state.value.sizes,
                "click-handler": fetch,
            ...
            return [
              ...
              !state.value.isLoading && unref(can)("show-translations") ? (openBlock(), createBlock("div", { key: 1 }, [
                unref(can)("create-translations") ? (openBlock(), createBlock(_sfc_main$3, {
                  key: 0,
                  type: "btn",
                  onClick: create
                }, {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Options/Translations/Index.vue");
