...
import _sfc_main$1 from "./Modal-58d6daab.mjs";
import _sfc_main$2 from "./OptionsLayout-49f01291.mjs";
import _sfc_main$3 from "./Table-58f2bc84.mjs";
import "./Form-20eb8b07.mjs";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    function fetch(page) {
      ...
      axios.get(route("api.xmls.index", { "page": state.value.currentPage })).then((response) => {
      ...
      if (can("destroy-xml")) {
        swal({
          ...
          if (result.isConfirmed) {
            axios.delete(route("api.xmls.destroy", id)).then(() => {
              fetch();
    ...
    function onEdit(id, i) {
      if (can("edit-xml")) {
        axios.get(route("api.xmls.edit", id)).then(({ data }) => {
    ...
    function onUpdate() {
      if (can("edit-xml")) {
        axios.put(route("api.xmls.update", state.value.item.id), state.value.item).then(() => {
          modalFunction();
          fetch();
          swal({
            title: "Success!",
    ...
    function onCreate() {
      if (can("create-xml")) {
        axios.post(route("api.xmls.create"), state.value.item).then(() => {
          modalFunction();
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$2, mergeProps({ title: "XML" }, _attrs), {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (!state.value.isLoading && unref(can)("show-xml")) {
              _push2(`<div${_scopeId}>`);
              if (unref(can)("create-xml")) {
              ...
              _push2(ssrRenderComponent(_sfc_main$3, {
                ...
                canDestroy: unref(can)("destroy-xml")
            ...
            return [
              ...
              !state.value.isLoading && unref(can)("show-xml") ? (openBlock(), createBlock("div", { key: 1 }, [
                unref(can)("create-xml") ? (openBlock(), createBlock(_component_button_component, {
                  key: 0,
                  type: "btn",
                  onClick: create
                }, {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Options/Xmls/Index.vue");
