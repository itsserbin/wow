...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const src = computed(() => (banner) => {
      ...
      return mobile ? mobile + ".jpeg" : table ? route("images.banners.table", table + ".jpeg") : desktop ? route("images.banners.desktop", desktop + ".jpeg") : null;
    });
    const srcset = computed(() => (banner) => {
      const mobile = banner.image_mobile[props.lang] || banner.image_mobile[defaultLang];
      const table = banner.image_table[props.lang] || banner.image_table[defaultLang];
      const desktop = banner.image_desktop[props.lang] || banner.image_desktop[defaultLang];
      const mobileSrc = mobile ? route("images.banners.mobile", mobile + ".webp") + " 568w," : route("images.banners.mobile", banner.image_mobile[defaultLang] + ".webp") + " 568w,";
      const tableSrc = table ? route("images.banners.table", table + ".webp") + " 991w," : route("images.banners.table", banner.image_table[defaultLang] + ".webp") + " 991w,";
      const desktopSrc = desktop ? route("images.banners.desktop", desktop + ".webp") : route("images.banners.desktop", banner.image_desktop[defaultLang] + ".webp");
