...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const src = computed(() => (banner) => {
      ...
      return mobile ? "/storage/banners/mobile/" + mobile + ".jpeg" : table ? "/storage/banners/table/" + table + ".jpeg" : desktop ? "/storage/banners/desktop/" + desktop + ".jpeg" : null;
    });
    const srcset = computed(() => (banner) => {
      const mobile = banner.image_mobile[props.lang] || banner.image_mobile[defaultLang];
      const table = banner.image_table[props.lang] || banner.image_table[defaultLang];
      const desktop = banner.image_desktop[props.lang] || banner.image_desktop[defaultLang];
      const mobileSrc = mobile ? "/storage/banners/mobile/" + mobile + ".webp 568w" : "/storage/banners/mobile/" + banner.image_mobile[defaultLang] + ".webp 568w";
      const tableSrc = table ? "/storage/banners/table/" + table + ".webp 568w" : "/storage/banners/table/" + banner.image_table[defaultLang] + ".webp 991w";
      const desktopSrc = desktop ? "/storage/banners/desktop/" + desktop + ".webp 568w" : "/storage/banners/desktop/" + banner.image_desktop[defaultLang] + ".webp";
