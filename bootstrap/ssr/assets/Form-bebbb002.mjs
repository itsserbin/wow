import { inject, ref, onMounted, mergeProps, unref, withCtx, createTextVNode, toDisplayString, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderList } from "vue/server-renderer";
import _sfc_main$9 from "./Images-02fe4913.mjs";
import { _ as _sfc_main$6 } from "./ImagesSelectModal-eeb13b5f.mjs";
...
import { _ as _sfc_main$4 } from "./ImageCard-96741a4a.mjs";
...
const _sfc_main = {
  ...
  setup(__props, { emit: emits }) {
    ...
    onMounted(async () => {
      ...
      await axios.get(route("api.categories.list")).then(({ data }) => state.value.categories = data.result).catch((response) => console.log(response));
      await axios.get(route("api.providers.list")).then(({ data }) => {
      ...
      await axios.get(route("api.colors.list")).then(({ data }) => state.value.colors = data.result).catch((response) => console.log(response));
      await axios.get(route("api.sizes.list")).then(({ data }) => state.value.sizes = data.result).catch((response) => console.log(response));
    });
